...
## Employee status: active, inactive, retired

Only a **live** employee can be selected inside an automation node. A new employee is **inactive** by default and stays invisible to the automation editor until you set it live from the [Playground](/help/ai-employees/going-live/test-in-playground) after testing.

| Status | Effect |
| ------ | ------ |
| **Live** | Selectable in automations and actively handling conversations wherever it is used. |
| **Inactive** | Stays in your list with its configuration intact, but stops running and disappears from automation node pickers until reactivated. |
| **Retired** | Permanent. Deletes all data inside the employee and removes it from every automation using it — this cannot be undone. |

<Frame>
  ![Employee status and Danger Zone settings](/assets/ai-employees-employee-status.png)
</Frame>

<Warning>
  Use **Set Inactive** to pause an employee without losing its setup. Only use **Retire**, under [**Settings** → Danger Zone](/help/ai-employees/capabilities/employee-settings#retire-an-ai-employee), when you are certain — there is no recovery afterward.
</Warning>
